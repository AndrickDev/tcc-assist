...
const MenuBar = ({ editor }: { editor: Editor | null }) => {
  ...
  const EditorButton = ({ onClick, isActive, disabled, children }: { onClick: () => void, isActive?: boolean, disabled?: boolean, children: React.ReactNode }) => (
    <button
      ...
      disabled={disabled}
      className={cn(
        "p-1.5 rounded transition-colors text-slate-400 hover:text-white hover:bg-[var(--brand-hover)]",
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 p-2 bg-[#1A1A18]/95 backdrop-blur-md border-b border-[var(--brand-border)] rounded-t-2xl sticky top-0 z-20 shadow-sm">
...
export function EditableRichText({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-[400px] p-8 sm:px-14 sm:py-12 text-[15.5px] sm:text-[16px] leading-[1.85] focus:outline-none text-[var(--brand-text)] font-serif max-w-none " +
          ...
          "[&_blockquote]:border-l-4 [&_blockquote]:border-orange-700/40 [&_blockquote]:pl-6 [&_blockquote]:pr-2 [&_blockquote]:py-1 [&_blockquote]:italic [&_blockquote]:my-6 [&_blockquote]:text-[var(--brand-muted)]",
      },
    },
  })
  ...
  return (
    <div className={cn("rounded-2xl overflow-hidden border border-[var(--brand-border)] bg-[var(--brand-hover)] shadow-brand flex flex-col", className)}>
      <MenuBar editor={editor} />
      <div className="flex-1 overflow-y-auto w-full custom-scroll bg-[var(--brand-hover)]">
