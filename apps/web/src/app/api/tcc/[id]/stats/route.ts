...
export async function GET(
  ...
  try {
    ...
    const tcc = await prisma.tcc.findFirst({
      ...
      include: {
        messages: {
          ...
          select: { content: true, agent: true }
        }
      }
    });

    if (!tcc) return NextResponse.json({ error: "Not found" }, { status: 404 });
    ...
    return NextResponse.json({ progress: totalProgress, plagiarism: plagiarismScore, humanAuthorship, totalPages, status: tcc.status, updatedAt: tcc.updatedAt });
  } catch (error) {
